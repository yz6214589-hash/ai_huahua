...
| 用户需求 | 主要技能 | 辅助技能 |
|---------|---------|---------|
| 写研报/深度分析/五步法 | write-report |web-search-qwen read-pdf, financial-analysis |
| 查最新股价/行情/走势 | stock-price | - |
| 搜最新新闻/政策/评级 |web-search-qwen| - |
| 读财报PDF/查报告数据 | read-pdf | - |
| 分析财务指标/ROE/负债率 | financial-analysis | read-pdf(获取数据) |
| 跨期对比/同行对比 | compare-reports | read-pdf(需要索引) |
| 舆情监控/事件驱动 | sentiment-analysis |web-search-qwen write-report |
| 恐慌指数/VIX/宏观风险 | sentiment-analysis |web-search-qwen|
| 预测市场/Polymarket/地缘政治 | sentiment-analysis |web-search-qwen|

## 技能协同规则

1. **写研报优先用 write-report**: 它包含完整的五步法框架和五种场景指南
2. **数据获取优先用web-search-qwen*: nanobot 内置的 web_search 用于实时搜索；`scripts/search_market.py` 用于 A 股专用搜索
