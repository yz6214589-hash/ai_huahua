...
from .common import JobStats, normalize_stock_code, safe_float
...
def _latest_dates(conn) -> dict[str, str]:
    """返回每只股票在 trade_stock_daily 中的最大日期，格式 YYYYMMDD。"""
...
def _get_stock_list(test_mode: bool, test_stock: str, max_stocks: int) -> tuple[list[str], dict[str, str]]:
    """获取待采集股票列表。test 模式只返回 test_stock；full 模式从 AkShare 获取全市场快照。"""
    if test_mode:
...
def _fetch_qmt(code: str, start: str, end: str = "") -> pd.DataFrame | None:
    """通过腾讯云 QMT Gateway HTTP 接口拉取日线，失败返回 None。"""
    try:
        from ai_quant_api.services.qmt_gateway_client import historical_kline
...
def _fetch_tushare(code: str, start: str, end: str = "") -> pd.DataFrame | None:
    """通过 Tushare 拉取日线，失败返回 None（需要 TUSHARE_TOKEN）。"""
...
def run_stock_daily(cfg: MySQLConfig, mode: str | None, params: dict[str, Any] | None) -> JobStats:
    """
    行情日线采集入口。
    数据源链路：QMT（xtquant） → AkShare → Tushare
    测试模式（mode=test）只采集 test_stock；全量模式（mode=full）最多采集 max_stocks 只股票。
    """
