...
def _mysql_upsert(rec: ReportTaskRecord) -> None:
    ...
    if not _mysql_enabled():
        return
    try:
        from core.db import connect, execute, load_mysql_config
...
_REPORT_TASKS_MYSQL_READY = False


def _ensure_report_tasks_table(conn) -> None:
    global _REPORT_TASKS_MYSQL_READY
    if _REPORT_TASKS_MYSQL_READY:
        return
    from core.db import execute
    ...
    """
    execute(conn, sql)
    _REPORT_TASKS_MYSQL_READY = True


def _mysql_upsert_report_tasks(rec: ReportTaskRecord) -> bool:
    """
    将任务记录写入 report_tasks 表（用户指定的 schema）。

    事务一致性策略：
    1. 先尝试 INSERT，失败时回滚
    2. 如果 INSERT 失败（重复主键）则 UPDATE
    3. 失败时返回 False，调用方负责删除已写入的文件
    ...
    try:
        ...
        return False
    try:
        _ensure_report_tasks_table(conn)
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        sql = """
        INSERT INTO report_tasks
          (id, created_at, status, report_path, model, use_rag, use_web,
           finish_time, updated_at, stock_codes, stock_names, mode, error_message)
        VALUES
          (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ON DUPLICATE KEY UPDATE
          status=VALUES(status),
        ...
        execute(
            ...
            (
                rec.task_id,
                _to_mysql_dt(rec.created_at) or now,
                rec.status or "",
                rec.report_path,
                rec.model or "",
                1 if rec.use_rag else 0,
                1 if rec.use_web else 0,
                _to_mysql_dt(getattr(rec, "finished_at", None)),
                now,
                json.dumps(rec.stock_codes or [], ensure_ascii=False),
                json.dumps(rec.stock_names or [], ensure_ascii=False),
                getattr(rec, "mode", "qwen") or "qwen",
                getattr(rec, "error_message", None),
            ),
        )
        return True
    except Exception:
...
def _mysql_delete_report_tasks(task_id: str) -> None:
    if not _mysql_enabled():
        return
    try:
        from core.db import connect, execute, load_mysql_config
    ...
    try:
        _ensure_report_tasks_table(conn)
        execute(conn, "DELETE FROM report_tasks WHERE id=%s", (task_id,))
    except Exception:
        return
    finally:
        conn.close()
...
def query_report_tasks(
    ...
    """
    从 report_tasks 表查询任务（支持多条件筛选）。

    Args:
        status_filter: 按 status 精确过滤
        model_filter: 按 model 模糊过滤（LIKE）
        created_start: 创建时间起点（ISO 字符串）
        created_end: 创建时间终点（ISO 字符串）
        limit: 最大返回条数
    ...
    try:
        ...
        if created_start:
            ...
            params.append(_to_mysql_dt(created_start))
        if created_end:
            conditions.append("created_at <= %s")
            params.append(_to_mysql_dt(created_end))
        ...
        for row in rows:
            row["use_rag"] = bool(row.get("use_rag"))
            row["use_web"] = bool(row.get("use_web"))
        return rows
    except Exception:
        return []
    finally:
...
def _persist(rec: ReportTaskRecord, *, mysql: bool = True) -> None:
    ...
    # 原子替换为目标文件
    tmp.replace(p)
    # 同步到 MySQL（双表：旧 ai_quant_report_tasks + 新 report_tasks）
    if mysql:
        _mysql_upsert(rec)
        _mysql_upsert_report_tasks(rec)
...
def create_task(
    ...
    rec = ReportTaskRecord(
        ...
        status="waiting",
        created_at=now_iso(),
        mode=str(mode or "qwen").strip() or "qwen",
...
def delete_task(task_id: str) -> bool:
    ...
    with _LOCK:
        ...
        if ok:
            ...
            _mysql_delete_report_tasks(task_id)
        return ok
