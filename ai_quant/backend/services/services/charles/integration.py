    ...
    try:
        ...
        rows = query_dict_func(
            ...
            SELECT
                ...
                t1.close_price,
                t1.stock_name,
                t2.close_price AS prev_close
            FROM (
                SELECT stock_code, trade_date, close_price, stock_name,
            ...
            LEFT JOIN (
                ...
                FROM trade_stock_daily
                WHERE stock_code IN ({placeholders})
            ) t2 ON t1.stock_code = t2.stock_code AND t2.rn = 2
            WHERE t1.rn = 1
            ORDER BY FIELD(t1.stock_code, {placeholders})
            """,
            [*codes, *codes, *codes],
        )
        ...
        for r in (rows or []):
            ...
            change = round(float(close - prev), 2) if close is not None and prev is not None else None
            pct = round(float((close - prev) / prev * 100), 2) if close is not None and prev is not None and prev != 0 else None
            items.append({
                "stock_code": code,
                "stock_name": r.get("stock_name"),
