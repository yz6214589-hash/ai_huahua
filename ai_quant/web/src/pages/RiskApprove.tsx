      ...
      <div className="mb-4 flex gap-1 border-b border-zinc-200">
        <button
          ...
          className={cn('border-b-2 px-4 py-2.5 text-sm font-medium transition',
            tab === 'approve' ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-800'
          )}
        >
          风控审批
        </button>
        <button
          ...
          className={cn('inline-flex items-center gap-1.5 border-b-2 px-4 py-2.5 text-sm font-medium transition',
            tab === 'design' ? 'border-zinc-900 text-zinc-900' : 'border-transparent text-zinc-500 hover:text-zinc-800'
          )}
        >
          <GitBranch className="h-4 w-4" /> 流程设计
        </button>
      </div>
      ...
      {tab === 'approve' && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
          <div className="lg:col-span-2">
            <Card>
              <CardHeader>
                <h3 className="text-sm font-semibold">风控审批</h3>
              </CardHeader>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input value={totalAsset} onChange={(e) => setTotalAsset(e.target.value)}
                        className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input value={atr14} onChange={(e) => setAtr14(e.target.value)}
                        className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400" />
                    </div>
                  </div>
                  <div>
                    <div className="mb-1 text-xs text-zinc-500">新闻/上下文（可选）</div>
                    <textarea value={newsText} onChange={(e) => setNewsText(e.target.value)} rows={3}
                      placeholder="填写相关市场新闻或上下文信息，有助于风控模型做出更准确判断"
                      className="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm outline-none focus:border-zinc-400" />
                  </div>
                  <button type="button" disabled={loading} onClick={run}
                    className={cn('inline-flex w-full items-center justify-center gap-2 rounded-lg bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-zinc-800 disabled:opacity-60')}>
          ...
          <div className="lg:col-span-3">
            <Card>
              <CardHeader>
                <h3 className="text-sm font-semibold">审批结果</h3>
              </CardHeader>
              <CardBody>
                {resp ? (
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge tone={tone(resp.decision) as any}>{resp.decision}</Badge>
                    </div>
                    <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-700">{resp.reason}</div>
                        ...
                        <div className="space-y-2">
                          {resp.checks.map((c, i) => (
                            <div key={i} className="flex items-start justify-between gap-3 rounded-lg border border-zinc-100 bg-white px-3 py-2 text-sm">
                              <div>
                                <Badge tone={tone(c.decision) as any}>{c.decision}</Badge>
