# -*- coding: utf-8 -*-
# 21-CASE-D: 板块轮动运行器 (从 CASE-B 复制核心逻辑)
"""
RotationRunner -- 整合"静态强度排名 + 一二阶导拐点检测", 给 graph.py 用

为什么不直接 import CASE-B:
    用户偏好"复制式独立"-- 4 个 CASE 完全自包含,
    晨会 CASE 拷贝到任何机器都能跑, 不依赖兄弟目录

复制源:
    - CASE-B/sector_loader.py        load_sector_index, list_sectors, build_market_benchmark
    - CASE-B/industry_strength.py    calc_strength_indicators, rank_industries
    - CASE-B/derivatives.py          calc_all_derivatives
    - CASE-B/inflection_detector.py  detect_one_sector_phase
    - CASE-B/run_today.py            build_combined_view (本文件的入口)

主接口:
    rank_industries_with_phase(level=2, lookback_days=90, top_n=5) -> List[Dict]
    get_sector_member_codes(sector_name, level=2) -> List[str]
"""
...
# ============================================================
# 板块清单 + 成分股 (复制自 industry_meta + sector_loader)
# ============================================================
...
# ============================================================
# 一阶导 + 二阶导 (复制自 derivatives + inflection_detector)
# ============================================================
...
# ============================================================
# 综合视图 (复制自 run_today.build_combined_view)
# ============================================================
...
def rank_industries_with_phase(level: int = 2,
    ...
    """
    给 graph.py 调用的主接口: 一次性返回 Top N 板块, 含静态强度 + 拐点 phase

    返回 DataFrame, index=sector_name, columns=[
        composite_score, composite_rank, score, rank,
