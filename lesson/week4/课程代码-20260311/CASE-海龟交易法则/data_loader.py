...
基于 CASE-Talib技术指标库 的 data_loader.py, 增加:
  - use_sizer 参数: 海龟策略使用自定义仓位管理, 无需默认 Sizer
"""
import pandas as pd
import numpy as np
...
def get_instrument_names(codes):
    """
    从 trade_stock_status 查询标的名称（股票/ETF 共用）

    参数:
        codes: 标的代码列表，如 ['600519.SH', '510300.SH']

    返回:
        dict {code: name}，未查到则用 code 作为 name
    """
    if not codes:
        return {}
    placeholders = ','.join(['%s'] * len(codes))
...
def run_and_report(strategy_class, stock_code, start_date=None, end_date=None,
    ...
    参数:
        ...
        use_sizer: 是否使用默认仓位管理器（海龟策略设为False）
