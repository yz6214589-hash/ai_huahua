...
class SimulationRequest(BaseModel):
    n_episodes: int = Field(default=1, ge=1, le=500)
    seed: int | None = None
...
class RLTrainRequest(BaseModel):
    ...
    start_date: str
    end_date: str
    total_qty: int = Field(default=100_000, ge=100)
    num_steps: int = Field(default=48, ge=1, le=390)
    impact_eta: float = Field(default=0.1, ge=0.0, le=10.0)
    impact_gamma: float = Field(default=0.05, ge=0.0, le=10.0)
    timesteps: int = Field(default=50_000, ge=1_000, le=5_000_000)
    model_out: str = Field(default="ethan/models/ppo_execution.zip")
...
class RLBacktestRequest(BaseModel):
    ...
    impact_eta: float = Field(default=0.1, ge=0.0, le=10.0)
    impact_gamma: float = Field(default=0.05, ge=0.0, le=10.0)
    n_episodes: int = Field(default=100, ge=1, le=2_000)
    rl_model_path: str | None = None
